function updateDepth(book, newPage) {
    ...
    if (newPage > 7) {
        ...
        $('.sensoro-book .p2 .depth').css({
            width: depthWidth,
            left: 20 - depthWidth
        });
    } else {
        $('.sensoro-book .p2 .depth').css({ width: 0 });
    }
    ...
    if (newPage < pages - 7) {
        $('.sensoro-book .p31 .depth').css({
            width: depthWidth,
            right: 20 - depthWidth
        });
    } else {
        $('.sensoro-book .p31 .depth').css({ width: 0 });
    }
}

function loadPage(page) {

    $.ajax({ url: '/zh/assets/lib/turn/pages/page' + page + '.html' }).
    done(function(pageHtml) {
        $('.sensoro-book .p' + page).html(pageHtml.replace('image/', '/zh/assets/lib/turn/'));
    });
...
function zoomThis(pic) {
    ...
    $('.sensoro-book').data().zoomIn = true;

    $('.sensoro-book').turn('disable', true);
...
function zoomOut() {
    ...
    var transitionEnd = $.cssTransitionEnd(),
        completeTransition = function(e) {
            $('#book-zoom').unbind(transitionEnd);
            $('.sensoro-book').turn('disable', false);
            $('body').css({ 'overflow': 'auto' });
            moveBar(false);
        };

    $('.sensoro-book').data().zoomIn = false;
...
function setPreview(view) {
    ...
    var previewWidth = 115,
        previewHeight = 73,
        previewSrc = '/zh/assets/lib/turn/pages/preview.jpg',
