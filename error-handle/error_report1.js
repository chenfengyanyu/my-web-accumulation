...
(function () {
  ...
  const EnvArr = [
    { env: 'local', str: 'localhost'},
    { env: 'testing', str: 'test.17zuoye.net' },
    { env: 'staging', str: 'staging.17zuoye.net' },
    { env: 'production', str: '17zuoye.com' }
  ];
  ...
  let jsErrorReport = (params) => {
    ...
    defaults.env = params.env || checkEnv();
  ...
  function report(url, data) {
    // 本地环境和 ip 访问不上报
    if(data.env === 'local' || data.env === 'unknow') return;
    let logParams = {
      _c: data.channel,
      _l: data.level,
  ...
  function checkEnv() {
    ...
    let index = EnvArr.findIndex((value, index, arr) => host.indexOf(value.str) > -1);
    return (EnvArr[index] && EnvArr[index].env) || 'unknow';
  }
  ...
  if (typeof window.__REPORT_CONFIG__ === 'object') {
    jsErrorReport(__REPORT_CONFIG__)
  }
})();
